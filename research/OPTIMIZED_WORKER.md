...
## Erkenntnisse 2026-09-20 (PHP und Wrapper)

- Vor einer Rückfrage den betroffenen Code lesen. Die Frage „wie streng soll die Prüfung sein" war durch einen Blick in `ConfigRenderer` beantwortet: der Wrapper ist generiert, geprüft wird nur das Snippet. Der Nutzer hat das zu Recht als unnötige Frage zurückgewiesen.
- Eigene Planvorgaben sind keine Tatsachen. Zweimal stand im Plan etwas, das der Bestandscode besser begründet anders löst (`ssl_certificate` über `cert/`, ACME im 443-Block). Der Kommentar an der Codestelle nannte den Grund – lesen schlägt planen.
- Aufzählungen in einer Fix-Vorgabe sind selbst eine Fehlerquelle. Bei der Loopback-Sperre hatte ich Adressbereiche aufgezählt und `::ffff:0.0.0.0` vergessen; bei der Sperrliste hatte ich `listen`/`server_name` vergessen. Besser die Regel nennen als die Fälle.
- nginx-Verhalten immer messen, nicht annehmen: `include fastcgi_params;` wird je nach Aufrufweg anders aufgelöst; `try_files` auf Server-Ebene erlaubt ein eigenes `location /`, ein generiertes `location /` verhindert es (mit `duplicate location` belegt).
- Rechte gegen den tatsächlichen Prozessbenutzer prüfen, nicht gegen die Tabelle: `logs/` mit `0750 root:<owner>` sah richtig aus, war für den PHP-Benutzer aber nicht durchquerbar.
- Dateisystem-Grenzen im Code über rechnerische Normalisierung prüfen, nicht über `realpath()`: die Zieldatei muss nicht existieren (`SnippetScope::isInside()`).
