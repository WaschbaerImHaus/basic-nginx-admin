...
- Derselbe fehlerhafte `chown`-Fallback wie im Migrator (übergibt `wwwGroup` als Benutzernamen statt eines gültigen Fallback-Benutzers) steckt noch in `VhostService::own()`/`applySpec()`; im Migrator wurde er beim Fix der Migration entfernt, hier bewusst zurückgestellt (siehe Plan-Ruling zu Task 6).
