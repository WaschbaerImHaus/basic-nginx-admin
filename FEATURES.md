...
- **Bienenflug passt seine Qualität der Bildrate an** (2026-09-25, Nutzerwunsch): Ein interner Zähler misst die Bilder je Sekunde (`data-fps` an `.meadow`). Ruckelt es zwei Sekunden lang (unter 42 B/s), sinkt die Qualität um eine Stufe: erst ohne Randunschärfe, dann ohne Gauß je Blume, dann 60 % und zuletzt 35 % der Blumen (gleichmässig ausgedünnt). Läuft es acht Sekunden flüssig (ab 56 B/s), kommt eine Stufe zurück; bricht es danach gleich wieder ein, bleibt die teurere Stufe gesperrt (kein Pendeln). Pausen und Tabwechsel zählen nicht als Ruckeln, die ersten zwei Sekunden auch nicht. `?quality=0…4` hält eine Stufe fest (Prüfbilder).
